...
#' @param compressionLevel is the netCDF compression level between 1 (low) and 9 (high), and \code{NA} for no compression. Note, data extracion runtime may slightly increase with the level of compression. The default is \code{5}.
...
#'    file.remove(ncdfFilename)
#' if (file.exists(ncdfSolarFilename))
#'    file.remove(ncdfSolarFilename)
#'
#' # Build netCDF grids for all data but only over a defined time period.
#' makeNetCDF_file(ncdfFilename=ncdfFilename, ncdfSolarFilename=ncdfSolarFilename,
